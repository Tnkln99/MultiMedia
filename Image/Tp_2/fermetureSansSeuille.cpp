...
bool minIntensiteVosisin(OCTET* Image, int i, int j, int nH, int nW) {
...
bool maxIntensiteVosisin(OCTET* Image, int i, int j, int nH, int nW) {
...
void dilate(OCTET* ImgIn, OCTET* ImgOut, int nH, int nW) {
    for (int i = 0; i < nH; i++) {
        for (int j = 0; j < nW; j++)
        {
            ImgIn[i * nW + j] = maxIntensiteVosisin(ImgIn, i, j, nH, nW);
        }
    }
}
...
int main(int argc, char* argv[])
{
    ...
    erose(ImgIn, ImgEnter, nH, nW);
    dilate(ImgEnter, ImgOut, nH, nW);
