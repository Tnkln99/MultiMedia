...
bool existVosisinNoir(OCTET* Image, int i, int j, int nH, int nW) {
    int Vkinit, Vkfin, Vpinit, Vpfin;
...
void dilate(OCTET* ImgIn, OCTET* ImgOut, int nH, int nW) {
    for (int i = 0; i < nH; i++) {
        for (int j = 0; j < nW; j++)
        {
...
void erose(OCTET* ImgIn, OCTET* ImgOut, int nH, int nW) {
    for (int i = 0; i < nH; i++) {
        for (int j = 0; j < nW; j++)
        {
            if (existVosisinNoir(ImgIn, i, j, nH, nW)) {
                //std::cout << "entrer" << std::endl;
                ImgOut[i * nW + j] = 0;
            }
            else {
...
int main(int argc, char* argv[])
{
    ...
    allocation_tableau(ImgEnter, OCTET, nTaille);
    //blach = arka plan
    //noir = objet

    //dilatation eger beyaz komsusu varsa o da beyaz oluyo obje buyutuyo. 
    //erosion eger siyah komsusu varsa o da siyah oluyo obejet kucultuyo
